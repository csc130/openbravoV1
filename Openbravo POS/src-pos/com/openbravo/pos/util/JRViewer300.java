		...
		if(elements != null && elements.size() > 0)
		{
			for(Iterator it = elements.iterator(); it.hasNext();)
			{
				...
				if (element instanceof JRPrintFrame)
				{
					...
					int frameOffsetX = offsetX + frame.getX(); //+ frame.getLineBox().getLeftPadding().intValue(); Tanes
					int frameOffsetY = offsetY + frame.getY(); //+ frame.getLineBox().getTopPadding().intValue(); Tanes
